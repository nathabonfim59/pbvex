...
Schema descriptors are optional, but without one there is no declared table authority for typed database access. PBVex v1 is single-node and does not provide vector search, arbitrary SQL, native join operators, or a general migration scripting API. Compose relationships explicitly as described in [Relationships and joins](./relationships-and-joins.md). Plan destructive data changes explicitly and back up before deployment changes.
