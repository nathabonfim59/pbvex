...
If these function kinds are new to you, start with [Backend primitives](../concepts/backend-primitives.md), which explains when to choose each kind and where its transaction and side-effect boundaries are.

Deployed code runs in Goja, not Node.js. Root functions cannot use `process.env` and do not receive `ctx.env`. Put code that requires server-provisioned configuration behind a component environment declaration; see [Environment variables and secrets](./environment-variables.md).
...
- `action` / `internalAction` receive `ActionCtx`: no direct database access; use `ctx.runQuery`, `ctx.runMutation`, `ctx.runAction`, or `ctx.run`. Actions also have auth, scheduler, storage, bounded [outbound HTTP](./outbound-http.md), and application [email template](./email-templates.md) capabilities.
...
Clients call only public query, mutation, and action references through `PBVexClient`; see [queries, mutations, and actions](./client/queries-mutations-actions.md). A query result may be subscribed to by the realtime client. PBVex conservatively reevaluates every active subscription after a successful record mutation and sends only results that changed.
