...
```tsx
import { useQuery } from '@pbvex/sdk-react';
...
`@pbvex/sdk-react` does not include a built-in optimistic-update layer. The client does not expose local cache mutation methods. For optimistic UI, maintain local state around the mutation call:

```tsx
import { useMutation, useQuery } from '@pbvex/sdk-react';
