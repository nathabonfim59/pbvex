...
`@pbvex/react` hooks are tested with `renderHook` and `PBVexProvider`. The core package tests use a mock `RealtimeTransport`.
...
```ts
import type { ConnectionState, QueryResult, RealtimeTransport, Unsubscribe, WatchOptions } from '@pbvex/client';
