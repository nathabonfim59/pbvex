...
`@pbvex/sdk-core` does not expose a direct file upload method. A server-side mutation or action first calls `ctx.storage.generateUploadUrl()` (see the [Storage guide](../../guides/storage.md)); the browser then uploads directly to that one-time URL.
...
```ts
import { Client, type StorageId, type StorageUploadResponse } from '@pbvex/sdk-core';
