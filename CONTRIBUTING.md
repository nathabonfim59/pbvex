...
- `packages/pbvex`: TypeScript authoring APIs, bundler, code generation, and
  CLI.
- `packages/client`: browser-neutral client and realtime transport.
- `packages/react` and `packages/svelte`: framework adapters.
...
- `docs`: user, self-hosting, protocol, and release documentation.
...
- The SDK `dist` files already tracked by Git are release artifacts. Rebuild
  ...
  intentional.
- Update user, self-hosting, protocol, or release documentation when a public
  contract changes.
