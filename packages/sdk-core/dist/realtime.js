import { decodeValue, canonicalJson, encodeValue, hashSha256, isSseEnvelope, isStructuredError, parseContentType, readBoundedText, DEFAULT_CONFIG, MAX_PATH_LENGTH, } from '@pbvex/protocol';
...
function isValidResponseBody(body) {
    return (body !== null &&
        typeof body === 'object' &&
...
class SseParser {
    ...
    pendingDataBytes = 0;
    discardEvent = false;
    discardErrorReported = false;
    callbacks;
    // The client-configured baseline limits; restored on reset() so each
    // connection re-negotiates from the safety ceiling.
    baselineMaxLineLength;
    baselineMaxEventDataLength;
    maxLineLength;
    maxEventDataLength;
    constructor(maxLineLength, maxEventDataLength) {
     ...
     */
    negotiateMaxEventDataLength(limit) {
    ...
    processLine(line) {
        ...
        if (this.pendingDataBytes + addedBytes > this.maxEventDataLength) {
...
function validateReconnectValue(value, fallback, name, max) {
    if (value === undefined)
        return fallback;
...
function validateDelay(value, fallback, name, max) {
    if (value === undefined)
        return fallback;
    if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0 || value > max || !Number.isInteger(value)) {
...
function validateWatchOptions(options) {
...
class Subscription {
    ...
    parser;
    isDisposed = false;
    latestResult = { data: undefined, error: null, isLoading: true };
    connectionId = 0;
    id = '';
    constructor(transport, subscriptionKey, path, args, encodedArgs, canonicalArgs, options) {
    ...
    addWatcher(options) {
        ...
        const watcher = { id: Subscription.nextWatcherId++, options: options };
    ...
    notifyConnectionStateChange(watcher) {
        try {
    ...
    async openConnection() {
        ...
        try {
            ...
            if (this.isDisposed || this.connectionId !== openConnectionId)
                return;
    ...
    processChunk(chunk, connectionId, done) {
        if (this.isDisposed || this.connectionId !== connectionId)
            return;
        this.parser.feed(chunk, done, {
            onData: (data) => this.handleEvent(data, connectionId),
            onError: (error) => this.reportError(error),
        });
    }
    handleEvent(data, connectionId) {
        if (this.isDisposed || this.connectionId !== connectionId)
            return;
        let envelope;
        try {
    ...
    scheduleReconnect(causeConnectionId) {
...
export class FetchRealtimeTransport {
    baseUrl;
    fetchFn;
    getAuthToken;
    realtimePath;
    ...
    async computeSubscriptionId(path, canonicalArgs) {
    ...
    watch(path, args, options) {
        ...
        if (!subscription) {
            subscription = new Subscription(this, subscriptionKey, path, args, encodedArgs, canonicalArgs, options);
