  ...
  try {
    ...
    for (const [modulePath, exports] of Object.entries(evaluatedModules)) {
      for (const [exportName, value] of Object.entries(exports)) {
        ...
        if (isCronJobsDefinition(value)) {
          if (modulePath !== `${options.entryDir ?? 'pbvex'}/crons.ts` || exportName !== 'default') {
            throw new Error('Recurring jobs must be the default export of pbvex/crons.ts');
          }
