...
export type { FunctionContext, AuthContext, UserIdentity, DatabaseReader, DatabaseWriter, SchedulerContext, SchedulableFunctionReference, JobId, JobID, StorageReader, StorageContext, StorageId, HttpContext, Request, Response, FunctionRoute } from 'pbvex/server';
...
export async function generateCodegenFiles(options: CodegenOptions, schema?: SchemaDefinition): Promise<void> {
  ...
  await Promise.all([
    ...
    writeFile(path.join(generatedDir, 'server.ts'), generateServerTs(), 'utf-8'),
  ]);
}
