...
import type { FunctionReference, StorageId } from 'pbvex/server';
...
export function generateDataModelTs(schema: SchemaDefinition | undefined): string {
  ...
  lines.push(`import type { GenericId, GenericDocument, TableInfo, StorageId } from 'pbvex/server';`);
  lines.push('');

  const tables = schema ? Object.keys(schema.tables).sort() : [];

  const taken = new Set<string>(['TableNames', 'Doc', 'Id', 'DataModel', 'GenericDocument', 'GenericId', 'TableInfo', 'StorageId']);
...
export type { FunctionContext, AuthContext, UserIdentity, DatabaseReader, DatabaseWriter, SchedulerContext, SchedulableFunctionReference, JobId, JobID, StorageReader, StorageContext, StorageId, StorageFileMetadata, StorageImageMetadata, StorageUploadResponse, EmailContext, EmailSendOptions, EmailTemplateVariables, HttpContext, HttpSendOptions, HttpSendResponse, Request, Response, FunctionRoute } from 'pbvex/server';
