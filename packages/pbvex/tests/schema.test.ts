    ...
    await writeFile(
      ...
      JSON.stringify({
        compilerOptions: {
          ...
          paths: {
            pbvex: [path.join(REPO_ROOT, 'packages/pbvex/src/runtime/index.ts')],
            'pbvex/server': [path.join(REPO_ROOT, 'packages/pbvex/src/runtime/server.ts')],
            'pbvex/values': [path.join(REPO_ROOT, 'packages/pbvex/src/runtime/values.ts')],
          },
        },
        include: ['schema.ts'],
      }, null, 2),
