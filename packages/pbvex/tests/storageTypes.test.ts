...
import type { ActionCtx, QueryCtx, SchedulerContext, StorageContext, StorageFileMetadata, StorageGetUrlOptions, StorageId, StorageImageMetadata, StorageReader } from '../src/runtime/server.js';
...
describe('storage authoring types', () => {
  it('uses branded ids and nullable getUrl results', () => {
    ...
    expectTypeOf<Parameters<StorageContext['getUrl']>[1]>().toEqualTypeOf<StorageGetUrlOptions | undefined>();
    expectTypeOf<Awaited<ReturnType<StorageContext['getUrl']>>>().toEqualTypeOf<string | null>();
    expectTypeOf<Awaited<ReturnType<StorageContext['getMetadata']>>>().toEqualTypeOf<StorageFileMetadata | StorageImageMetadata | null>();
