...
pnpm --dir packages/sdk-core pack --pack-destination "$PACK_DIR"
pnpm --dir packages/sdk-react pack --pack-destination "$PACK_DIR"
pnpm --dir packages/sdk-svelte pack --pack-destination "$PACK_DIR"

protocol_tgz=("$PACK_DIR"/pbvex-protocol-*.tgz)
pbvex_tgz=("$PACK_DIR"/pbvex-*.tgz)
core_tgz=("$PACK_DIR"/pbvex-sdk-core-*.tgz)
react_tgz=("$PACK_DIR"/pbvex-sdk-react-*.tgz)
svelte_tgz=("$PACK_DIR"/pbvex-sdk-svelte-*.tgz)
...
for archive in "${protocol_tgz[0]}" "$pbvex_package" "${core_tgz[0]}" "${react_tgz[0]}" "${svelte_tgz[0]}"; do
...
import { createQuery } from '@pbvex/sdk-svelte';
...
import { v } from 'pbvex/values';
import { Client } from '@pbvex/sdk-core';
import * as reactSdk from '@pbvex/sdk-react';
import * as svelteSdk from '@pbvex/sdk-svelte';
