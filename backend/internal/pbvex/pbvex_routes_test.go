...
func TestFullRegisterPreservesPocketBaseAuthenticationRoutes(t *testing.T) {
	app, _ := newTestApp(t)
	router, err := apis.NewRouter(app)
	if err != nil {
		t.Fatal(err)
	...
	for _, route := range []struct {
		...
		{http.MethodGet, "/api/collections/{collection}/auth-methods"},
		{http.MethodPost, "/api/collections/{collection}/auth-with-password"},
		{http.MethodPost, "/api/collections/{collection}/request-otp"},
		{http.MethodPost, "/api/collections/{collection}/auth-with-otp"},
		{http.MethodPost, "/api/collections/{collection}/auth-with-oauth2"},
		{http.MethodPost, "/api/collections/{collection}/auth-refresh"},
		{http.MethodPost, "/api/collections/{collection}/impersonate/{id}"},
		{http.MethodGet, "/api/oauth2-redirect"},
		{http.MethodPost, "/api/oauth2-redirect"},
	} {
		t.Run(route.method+" "+route.path, func(t *testing.T) {
			if !router.HasRoute(route.method, route.path) {
				t.Fatalf("PocketBase auth route was lost after PBVex registration")
			}
		})
	}
}
