...
// TestRealtimeSDKCoreE2E runs a real @pbvex/sdk-core FetchRealtimeTransport
...
func TestRealtimeSDKCoreE2E(t *testing.T) {
	...
	sdkCoreDist := filepath.Join(repoRoot, "packages", "sdk-core", "dist", "index.js")
	...
	buildCmd := exec.Command(pnpm, "--filter", "@pbvex/sdk-core", "build")
	buildCmd.Dir = repoRoot
	buildOut, err := buildCmd.CombinedOutput()
	if err != nil {
		t.Fatalf("sdk-core build failed: %v\n%s", err, string(buildOut))
	}

	if _, err := os.Stat(sdkCoreDist); err != nil {
		t.Fatalf("sdk-core dist missing after build: %s", sdkCoreDist)
	}
...
`, "file://"+sdkCoreDist)
	...
	cmd.Dir = filepath.Join(repoRoot, "packages", "sdk-core")
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "STATE ") {
			t.Logf("sdk-core state: %s", strings.TrimPrefix(line, "STATE "))
			continue
		}
		if strings.HasPrefix(line, "ERROR ") {
			t.Fatalf("sdk-core error: %s", strings.TrimPrefix(line, "ERROR "))
		}
