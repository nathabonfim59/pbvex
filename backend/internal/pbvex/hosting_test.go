...
func TestHostingSettingsAndRestore(t *testing.T) {
	...
	go server.Serve(l)
	defer server.Close()
	if err := registerHosting(app, hosting.Config{Enabled: true, SocketPath: path}); err != nil {
		t.Fatal(err)
	}
...
func TestHostingDisabledAndUnavailable(t *testing.T) {
	...
	defer app.Cleanup()
	if err := registerHosting(app, hosting.Config{}); err != nil {
		t.Fatal(err)
	}
	if err := registerHosting(app, hosting.Config{Enabled: true, SocketPath: filepath.Join(t.TempDir(), "absent")}); err == nil {
